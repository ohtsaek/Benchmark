...
for SPLIT_IDX in 1 2 3 4 5; do
  python train.py \
    ...
    || {
      echo "[WARN] train.py failed: PRISM cell_blind split_idx=${SPLIT_IDX}" >&2
      true
    }
done
...
#   data/PRISM/Experiment/cell_blind/results/split<split_idx>_initseed<init_seed>_<timestamp>/
# --dataset_name PRISM: uses split_type and split_idx to load
#   cell_blind_<split_idx>_{Training,Validation,Test}.csv under data_split/.
...
# --split_type cell_blind: held-out cell lines; this script runs split_idx 1..5.
